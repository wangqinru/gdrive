...
func (self *Drive) listAllFiles(args listAllFilesArgs) ([]*drive.File, error) {
	...
	err := self.service.Files.List().Q(args.query).SupportsAllDrives(true).Fields(args.fields...).OrderBy(args.sortOrder).PageSize(pageSize).Pages(context.TODO(), func(fl *drive.FileList) error {
